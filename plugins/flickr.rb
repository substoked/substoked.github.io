...
class FlickrSimplePhotoHtml
  ...
  def toHtml
    aAttrs = {href: @src, title: @title}
    imgAttrs = {src: @src, title: @title, alt: @desc}
    imgCssAttrs = {}
...
class GallerySetsTag < Liquid::Tag
  ...
  def initialize(tag_name, markup, tokens)
    ...
    def render(context)
    getHtml(@ids, @size, @showSetDesc)
  end

  def getHtml(ids, size, showSetDesc)
    ...
    idList.each do |id|
      ...
      info1 = flickrCached.photosets.getInfo(photoset_id: id, extras: apiExtras)
      sets = FlickRaw.url_photosets(info1) # => "https://www.flickr.com/photos/41650587@N02/sets/"
      ...
      response['photo'].each do |photo|
        ...
        if photo['media'] == 'video'
          ...
          html = FlickrSimplePhotoHtml.new(photo["id"], params).toHtml
        end
        setPhotosHtml.push(html)
      end
    end

    setHtml = '<div class="flickr-set">' + setPhotosHtml.join + '</div>'
...
Liquid::Template.register_tag("flickr_gallery", GallerySetsTag)